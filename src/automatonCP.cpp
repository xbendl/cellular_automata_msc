...
AutomatonCP::AutomatonCP(GridType &zygote, ChromosomeSet &chromosomeSet, E_NEIGHBOURHOOD neighbourhood)
{
...
void AutomatonCP::AutomatonStep(int stepCount)
{
    ...
    for(int i = 0; i < stepCount; i++)
    {
        ...
        for(int row = 0; row < config->gridSize; row++)
        {
            for(int col = 0; col < config->gridSize; col++)
            {
                ...
                for(int j = 0; j < (int)config->instructionCount; j++)
                {
                    ...
                    else if(chromosomeSet[cellNumber].instrList[j]->name == "NOP")
                    {   // NOP instrukce nejsou vyhodnocovany
                        continue;
                    }
                    SetNewConfiguration(mapGrid, cellNumber, newGrid, chromosomeSet[cellNumber].instrList[j]);
                }
            }
        }
        this->grid = newGrid;
...
void AutomatonCP::AutomatonStep(int stepCount, std::vector<int> &cellPossibilities, double damageProbability)
{
    // Provedeni prechodu
...
void AutomatonCP::SetNewConfiguration(GridType &mapGrid, int cellNumber, GridType &newGrid, Instruction *actInstruction)
{
    ...
    int arraySize;
    if(config->neighbourhood == NEIGHBOURHOOD_VON_NEUMANN)
        arraySize = 5;
    else
        arraySize = 9;
    CellType actConfig[arraySize];
...
Automaton::CellType AutomatonCP::GetNextState(CellType actConfig[], Instruction *actInstruction)
{
